...
#include "Basic/KMeanLocal/KMlocal.h" 
...
namespace WBSF
{
	...
	const char* CWeatherGenerationApp::XML_FLAG = "WeatherGeneration";
	const char* CWeatherGenerationApp::MEMBERS_NAME[NB_MEMBERS] = { "WGInputName", "ParametersVariationsName", "NbReplications" };
	//const int CWeatherGenerationApp::CLASS_NUMBER = CExecutableFactory::RegisterClass(CWeatherGenerationApp::GetXMLFlag(), &CWeatherGenerationApp::CreateObject);


	CWeatherGenerationApp::CWeatherGenerationApp()
	{
		Reset();
	}

	CWeatherGenerationApp::CWeatherGenerationApp(const CWeatherGenerationApp& in)
	{
		operator=(in);
	}
	...
	void CWeatherGenerationApp::Reset()
	{
	...
	CWeatherGenerationApp& CWeatherGenerationApp::operator =(const CWeatherGenerationApp& in)
	{
		if (&in != this)
		{
	...
	bool CWeatherGenerationApp::operator == (const CWeatherGenerationApp& in)const
	{
		bool bEqual = true;
	...
	ERMsg CWeatherGenerationApp::GenerateLocationListFromWeatherStation(const CFileManager& fileManager, const CWGInput& WGInput, const CLocationVector& locations, CLocationVector& stations)
	{
		ERMsg msg;
	...
	//mettre dans les DB
	ERMsg CWeatherGenerationApp::CheckLocationsInDatabase(CNormalsDatabasePtr& pNormalDB, CDailyDatabasePtr& pDailyDB, CHourlyDatabasePtr& pHourlyDB, const CLocationVector& locations, const CWGInput& WGInput, CCallback& callback)
	{
		ERMsg msg;
...
#pragma omp flush(messageTmp)
	...
	ERMsg CWeatherGenerationApp::Execute(const CFileManager& fileManager, CCallback& callback)
	{
		ASSERT(m_bExecute);
	...
	ERMsg CWeatherGenerationApp::GenerateWeather(CNormalsDatabasePtr& normalDB, CDailyDatabasePtr& dailyDB, CHourlyDatabasePtr& hourlyDB,
...
#pragma omp flush(msg)
	...
	void CWeatherGenerationApp::GetLocationIndexGrid(const CLocationVector& locArray, vector<size_t>& locPos)
	{
	...
	/*void CWeatherGenerationApp::writeStruc(zen::XmlElement& output)const
	{
		CExecutable::writeStruc(output);

		zen::XmlOut out(output);

		out[CWeatherGenerationApp::GetMemberName(CWeatherGenerationApp::WG_INPUT_NAME)](m_WGInputName);
		out[CWeatherGenerationApp::GetMemberName(CWeatherGenerationApp::LOCATIONS_NAME)](m_locationsName);
		out[CWeatherGenerationApp::GetMemberName(CWeatherGenerationApp::PVARIATIONS_NAME)](m_parametersVariationsName);
		out[CWeatherGenerationApp::GetMemberName(CWeatherGenerationApp::NB_REPLICATIONS)](m_nbReplications);
		out[CWeatherGenerationApp::GetMemberName(CWeatherGenerationApp::USE_HXGRID)](m_bUseHxGrid);
	}

	bool CWeatherGenerationApp::readStruc(const zen::XmlElement& input)
	{
		...
		zen::XmlIn in(input);
		in[CWeatherGenerationApp::GetMemberName(CWeatherGenerationApp::WG_INPUT_NAME)](m_WGInputName);
		in[CWeatherGenerationApp::GetMemberName(CWeatherGenerationApp::LOCATIONS_NAME)](m_locationsName);
		in[CWeatherGenerationApp::GetMemberName(CWeatherGenerationApp::PVARIATIONS_NAME)](m_parametersVariationsName);
		in[CWeatherGenerationApp::GetMemberName(CWeatherGenerationApp::NB_REPLICATIONS)](m_nbReplications);
		in[CWeatherGenerationApp::GetMemberName(CWeatherGenerationApp::USE_HXGRID)](m_bUseHxGrid);
